...
-- 1. Add image support to the base table
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS image_url text;

-- 2. Update the monitor view (Preserving your existing calculation logic)
-- We drop and recreate to ensure the new columns are available to the UI.
DROP VIEW IF EXISTS public.inventory_monitor_view CASCADE;
